...
) : CallAdapter<Type, Call<FakeResponse<Type>>> {
    override fun responseType() = type
    override fun adapt(call: Call<Type>): Call<FakeResponse<Type>> = ResultCall(call)
}
...
class ResultCall<T>(proxy: Call<T>) : CallDelegate<T, FakeResponse<T>>(proxy) {
    override fun enqueueImpl(callback: Callback<FakeResponse<T>>) {
        proxy.enqueue(object : Callback<T> {
            override fun onResponse(call: Call<T>, response: Response<T>) {

                val tmdbResponse = if (response.isSuccessful) {
                    FakeResponse.Success(response.body())
            ...
            override fun onFailure(call: Call<T>, t: Throwable) {
                val tmdbResponse = FakeResponse.Failure(
                        statusCode = -1,
                        statusMessage = "",
